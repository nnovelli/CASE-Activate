...
<pre>
  ...
  model ICS_Skeleton_wStorage
    ...
    parameter Modelica.SIunits.Pressure PAmb(displayUnit = "PA") = 101325
      "Ambient pressure";
    Modelica.Thermal.FluidHeatFlow.Sources.VolumeFlow Pump(constantVolumeFlow = 2e-006, medium = Modelica.Thermal.FluidHeatFlow.Media.Water(), m = 0.1,
      T0=293) "Fluid pump for thermal fluid"                                                                                                     annotation(Placement(visible = true, transformation(origin={-46,-20},   extent = {{-10,-10},{10,10}}, rotation = 0)));
    ICSolar.ICS_Context ics_context1 annotation(Placement(visible = true, transformation(origin = {-80,40}, extent = {{-25,-25},{25,25}}, rotation = 0)));
    ICSolar.Envelope.ICS_EnvelopeCassette ics_envelopecassette1 annotation(Placement(visible = true, transformation(origin = {0,40}, extent = {{-25,-25},{25,25}}, rotation = 0)));
    ...
    Modelica.Thermal.FluidHeatFlow.Sources.Ambient ambient(
        constantAmbientPressure(displayUnit="kPa") = 101300)
      "baseline pressure for loop"
      annotation (Placement(transformation(extent={{-74,-12},{-94,8}})));
  equation
    connect(heatedPipe.flowPort_b,Pump.flowPort_a) annotation(Line(points={{60,-28},
            {60,-52.0669},{-65.2636,-52.0669},{-65.2636,-20},{-56,-20}},                                                                                     color = {255,0,0}));
    ...
    connect(Pump.flowPort_b,ics_envelopecassette1.flowport_a) annotation(Line(points={{-36,-20},
            {-28.3554,-20},{-28.3554,19.6597},{-23.75,19.6597},{-23.75,18.75}}));
    connect(ics_context1.DNI,ics_envelopecassette1.DNI) annotation(Line(points = {{-55,25},{-24.1966,25},{-25,25}}));
    connect(ics_context1.AOI,ics_envelopecassette1.AOI) annotation(Line(points = {{-55,30},{-24.9527,30},{-25,30}}));
    connect(ics_context1.SunAzi,ics_envelopecassette1.SunAzi) annotation(Line(points = {{-55,35},{-24.9527,35},{-25,35}}));
    connect(ics_context1.SunAlt,ics_envelopecassette1.SunAlt) annotation(Line(points = {{-55,40},{-24.5747,40},{-25,40}}));
